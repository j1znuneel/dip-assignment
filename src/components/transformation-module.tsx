...
const KERNEL_BUFFER_R = new Uint8Array(256)
const KERNEL_BUFFER_G = new Uint8Array(256)
const KERNEL_BUFFER_B = new Uint8Array(256)
...
export function TransformationModule({ id, title, description, theory, formula, pythonCode, image }: ModuleProps) {
  ...
  const processImage = () => {
    ...
    const spectrumCtx = spectrumRef.current?.getContext('2d')
    if (!ctx) return
    ...
    img.onload = () => {
      ...
      const data = new Uint8ClampedArray(imageData.data) // Use copy for spatial
      const outputData = ctx.createImageData(w, h)
      const output = outputData.data

      // NOISE ADDITION (For restoration modules)
      ...
      if (isFreq) {
        ...
        if (spectrumCtx) {
          const specData = spectrumCtx.createImageData(w, h)
          for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
              const mag = Math.sqrt(re[y * N + x]**2 + im[y * N + x]**2)
              const val = Math.log(1 + mag) * 15, sidx = (y * w + x) * 4
              specData.data[sidx] = specData.data[sidx+1] = specData.data[sidx+2] = val; specData.data[sidx+3] = 255
            }
          }
          spectrumCtx.putImageData(specData, 0, 0)
        }
        ...
        for (let y = 0; y < h; y++) {
          for (let x = 0; x < w; x++) {
            const idx = (y * w + x) * 4
            let res = re[y * N + x] * (((x + y) % 2 === 0) ? 1 : -1)
            res = Math.max(0, Math.min(255, res))
            output[idx] = output[idx+1] = output[idx+2] = res; output[idx+3] = 255
          }
        }
      } else {
        const isPoint = ['negative', 'log', 'power', 'threshold', 'hist_eq'].includes(id)
        if (isPoint) {
          ...
          for (let i = 0; i < data.length; i += 4) {
            output[i] = lut[data[i]]; output[i+1] = lut[data[i+1]]; output[i+2] = lut[data[i+2]]; output[i+3] = 255
          }
        } else {
          ...
          for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
              ...
              if (id === 'sobel') {
                ...
                for (let i = -1; i <= 1; i++) {
                  for (let j = -1; j <= 1; j++) {
                    const py = Math.max(0, Math.min(h-1, y+i)), px = Math.max(0, Math.min(w-1, x+j))
                    const v = (data[(py*w+px)*4] + data[(py*w+px)*4+1] + data[(py*w+px)*4+2])/3
                    rX += v * gx[i+1][j+1]; rY += v * gy[i+1][j+1]
                  }
                }
                const m = Math.sqrt(rX*rX + rY*rY); output[idx] = output[idx+1] = output[idx+2] = m; output[idx+3] = 255
                ...
                for (let i = -1; i <= 1; i++) {
                  for (let j = -1; j <= 1; j++) {
                    const py = Math.max(0, Math.min(h-1, y+i)), px = Math.max(0, Math.min(w-1, x+j))
                    r += data[(py*w+px)*4] * k[i+1][j+1]
                  }
                }
                r = Math.max(0, Math.min(255, r)); output[idx] = output[idx+1] = output[idx+2] = r; output[idx+3] = 255
                ...
                for (let i = -offset; i <= offset; i++) {
                  for (let j = -offset; j <= offset; j++) {
                    ...
                    KERNEL_BUFFER_R[count] = data[pidx]; KERNEL_BUFFER_G[count] = data[pidx+1]; KERNEL_BUFFER_B[count] = data[pidx+2]
                    count++
                  }
                }
                if (id === 'median') {
                  const sortedR = KERNEL_BUFFER_R.subarray(0, count).sort(), sortedG = KERNEL_BUFFER_G.subarray(0, count).sort(), sortedB = KERNEL_BUFFER_B.subarray(0, count).sort()
                  const mid = Math.floor(count / 2)
                  output[idx] = sortedR[mid]; output[idx+1] = sortedG[mid]; output[idx+2] = sortedB[mid]
                } else if (id === 'max') {
                  let mr = 0, mg = 0, mb = 0
                  for (let i = 0; i < count; i++) { if (KERNEL_BUFFER_R[i] > mr) mr = KERNEL_BUFFER_R[i]; if (KERNEL_BUFFER_G[i] > mg) mg = KERNEL_BUFFER_G[i]; if (KERNEL_BUFFER_B[i] > mb) mb = KERNEL_BUFFER_B[i] }
                ...
                const w_val = 1 / (ksize * ksize)
                let r = 0, g = 0, b = 0
                for (let i = -offset; i <= offset; i++) {
                  for (let j = -offset; j <= offset; j++) {
                    const py = Math.max(0, Math.min(h-1, y+i)), px = Math.max(0, Math.min(w-1, x+j))
                    const pidx = (py * w + px) * 4
                    r += data[pidx] * w_val; g += data[pidx+1] * w_val; b += data[pidx+2] * w_val
                  }
                }
    ...
    <motion.div key={id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex flex-col gap-8 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 border-b border-white/[0.05] pb-8">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => setShowCode(true)} className="gap-2 border-white/[0.08] bg-white/[0.02] text-xs h-9 px-4"><Code2 className="size-3.5" />Show Code</Button>
        </div>
      </div>

      <Dialog open={showCode} onOpenChange={setShowCode}>
        <DialogContent className="max-w-2xl bg-[#08090a] border-white/[0.1] p-0 overflow-hidden shadow-2xl">
          <DialogHeader className="px-6 py-4 border-b border-white/[0.05] bg-white/[0.02] flex-row justify-between">
            <div className="flex items-center gap-2.5"><Terminal className="size-4 text-emerald-500" /><div><DialogTitle className="text-sm font-bold text-white/90">Python Implementation</DialogTitle></div></div>
            <Button variant="outline" size="sm" onClick={copyToClipboard} className="h-8 text-xs">{copied ? "Copied" : "Copy Code"}</Button>
          </DialogHeader>
          <ScrollArea className="h-[400px] w-full bg-[#050505]"><pre className="p-8 text-[13px] font-mono text-emerald-400/80 leading-relaxed"><code>{pythonCode}</code></pre></ScrollArea>
        </DialogContent>
      </Dialog>
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        <div className="lg:col-span-8 space-y-8">
          <div className="flex p-1 bg-white/[0.03] rounded-lg border border-white/[0.05] w-fit shadow-inner">
            <Button variant={activeTab === 'preview' ? 'secondary' : 'ghost'} size="sm" onClick={() => setActiveTab('preview')} className="px-6 h-8 text-xs">Workspace</Button>
            <Button variant={activeTab === 'theory' ? 'secondary' : 'ghost'} size="sm" onClick={() => setActiveTab('theory')} className="px-6 h-8 text-xs">Theorems</Button>
          </div>

          <div className={activeTab === 'preview' ? 'block' : 'hidden'}>
            <div className="grid gap-8 md:grid-cols-2">
              <div className="space-y-3"><label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/50">Source Stream</label><div className="aspect-square rounded-2xl border border-white/[0.05] bg-[#0a0a0a] overflow-hidden shadow-2xl">{image && <img src={image} className="h-full w-full object-contain p-8" />}</div></div>
              <div className="space-y-3">
                <div className="flex justify-between items-center"><label className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-500/60">Output Stream</label><button onClick={processImage} className="text-muted-foreground hover:text-emerald-400 p-1"><RefreshCw className={`size-3 ${processing ? 'animate-spin' : ''}`} /></button></div>
              ...
              {id.includes('_lpf') || id.includes('_hpf') ? (
                <div className="md:col-span-2 space-y-3 mt-4">
                  <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-500/60">Centered FFT Spectrum (u,v)</label>
                  <div className="h-48 w-full rounded-2xl border border-white/[0.05] bg-[#0a0a0a] overflow-hidden shadow-2xl flex items-center justify-center bg-gradient-to-br from-[#0a0a0a] to-[#050505]">
                    <canvas ref={spectrumRef} className="h-full opacity-80" />
                  </div>
                </div>
              ) : null}
            </div>
          ...
          <div className={activeTab === 'theory' ? 'block' : 'hidden'}>
            <Card className="border-white/[0.05] bg-white/[0.02] backdrop-blur-sm rounded-2xl shadow-2xl"><CardContent className="p-10 space-y-10">
              <div className="space-y-6"><div className="flex items-center gap-3 text-white/90"><BookOpen className="size-4 text-emerald-500" /><h3 className="font-bold text-lg tracking-tight">Theory</h3></div><p className="text-muted-foreground text-lg italic">"{theory}"</p></div>
              <div className="p-10 rounded-2xl bg-black border border-white/[0.05] shadow-inner"><div className="absolute top-4 left-5 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/30">Base Formula</div><div className="text-3xl font-mono text-center py-6 text-white tracking-tighter overflow-x-auto whitespace-nowrap">{formula}</div></div>
            </CardContent></Card>
          </div>
        </div>
              ...
              <div className="space-y-8">
                {id.includes('_lpf') || id.includes('_hpf') ? (
                  <div className="space-y-6">
                    <div className="space-y-4"><div className="flex justify-between items-center"><span className="text-[11px] font-bold text-white/60 uppercase tracking-[0.15em]">Cutoff Radius (D0)</span><span className="text-xs font-mono bg-white/[0.05] border border-white/[0.08] text-white px-3 py-1.5 rounded-md">{cutoff}</span></div><input type="range" min="5" max="250" step="1" value={cutoff} onChange={(e) => setCutoff(parseInt(e.target.value))} className="w-full accent-emerald-500 h-[3px] bg-white/[0.05] rounded-full appearance-none cursor-pointer" /></div>
                    {id.includes('butter') && (<div className="space-y-4"><div className="flex justify-between items-center"><span className="text-[11px] font-bold text-white/60 uppercase tracking-[0.15em]">Order (n)</span><span className="text-xs font-mono bg-white/[0.05] border border-white/[0.08] text-white px-3 py-1.5 rounded-md">{order}</span></div><input type="range" min="1" max="10" step="1" value={order} onChange={(e) => setOrder(parseInt(e.target.value))} className="w-full accent-emerald-500 h-[3px] bg-white/[0.05] rounded-full appearance-none cursor-pointer" /></div>)}
                  </div>
                ) : (
                  <div className="space-y-6">
                    {['box_blur', 'median', 'max', 'min'].includes(id) && (
                    ...
                    {['median', 'max', 'min'].includes(id) && (
                      <div className="space-y-4">
                        <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-white/60 uppercase tracking-[0.15em]">Noise Density</span><span className="text-xs font-mono bg-white/[0.05] border border-white/[0.08] text-white px-3 py-1.5 rounded-md">{(noiseLevel * 100).toFixed(0)}%</span></div>
                ...
                <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-[11px] text-emerald-400/80 italic font-medium leading-relaxed flex gap-3"><Activity className="size-4 shrink-0" /> Localized {id.includes('_') ? 'Frequency' : 'Spatial'} hardware acceleration active.</div>
              </div>
